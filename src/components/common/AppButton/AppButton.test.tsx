    ...
    if (!ignoreClassName) {
      ...
      expect(
        button?.className?.includes(
          `MuiButton-contained${capitalize(expectedClassName)}`,
        ),
      ).toBeTruthy(); // Check for "MuiButton-contained[Primary| Secondary |...]" class
    }
  });
}
...
describe("<AppButton/> component", () => {
  ...
  it("supports .startIcon property as <svg/>", () => {
    ...
    render(
      <ComponentToTest
        text={text}
        startIcon={<DefaultIcon data-testid="startIcon" />}
      />,
    );
  ...
  it("supports .endIcon property", () => {
    ...
    render(
      <ComponentToTest
        text={text}
        endIcon={<DefaultIcon data-testid="endIcon" />}
      />,
    );
    let button = screen.getByText(text);
